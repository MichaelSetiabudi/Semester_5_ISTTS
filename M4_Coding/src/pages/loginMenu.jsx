...
function LoginMenu({
  ...
  const updateUserBookProgress = async (bookId, lastPages) => {
    try {
      const response = await fetch(`http://localhost:3000/api/users/${user_id}/books/${bookId}`, {
        method: 'PUT',
        headers: {
  ...
  const allUsersBook = getAllBooksSortedByLastRead(userBooks);
